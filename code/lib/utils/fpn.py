...
def map_rois_to_fpn_levels(rois, k_min, k_max):
    ...
    # print(len(target_lvls), rois.shape)
...
def add_multilevel_roi_blobs(
    ...
    # print('rois', blob_prefix, rois.shape)
    for lvl in range(lvl_min, lvl_max + 1):
        idx_lvl = np.where(target_lvls == lvl)[0]
        for i in range(lvl_min, lvl_max+1):
            if lvl == lvl_min:
                blobs[blob_prefix + '_fpn' + str(i)] = rois[idx_lvl, :]
            else:
                # print(type(rois[idx_lvl, :]))
                blobs[blob_prefix + '_fpn' + str(i)] = np.concatenate(
                                                    (blobs[blob_prefix + '_fpn' + str(i)],
                                                    rois[idx_lvl, :]), axis=0)

        # print('lvl_', lvl, rois[idx_lvl, :].shape)
        rois_idx_order = np.concatenate((rois_idx_order, idx_lvl))
        # print('before', rois_stacked.shape)
        # rois_stacked = np.vstack(
        #     [rois_stacked, blobs[blob_prefix + '_fpn' + str(lvl)]]
        # )
        # print('after', rois_stacked.shape)
    ...
    for lvl in range (lvl_min, lvl_max+1):
        rois_stacked = blobs[blob_prefix + '_fpn' + str(lvl)]
        # print(blob_prefix, lvl, rois_stacked.shape)

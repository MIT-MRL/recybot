...
class Generalized_RCNN(nn.Module):
    def __init__(self):
        ...
        if cfg.MODEL.MASK_ON:
            ...
            self.Mask_Outs = mask_rcnn_heads.mask_rcnn_edge_outputs(self.Mask_Head.dim_out)
    ...
    def _forward(self, data, im_info, roidb=None, **rpn_kwargs):
        ...
        if self.training:
            ...
            if cfg.MODEL.MASK_ON:
                ...
                loss_mask = mask_rcnn_heads.mask_rcnn_edge_losses(mask_pred, rpn_ret['masks_int32'])
    ...
    def roi_feature_transform(self, blobs_in, rpn_ret, blob_rois='rois', method='RoIPoolF',
        ...
        if isinstance(blobs_in, list):
                    ...
                    if method == 'RoIPoolF':
                        ...
                        if isinstance(resolution, list):
                            xform_out = RoIAlignFunction(
                                resolution[k_max-lvl], resolution[k_max-lvl], sc, sampling_ratio)(bl_in, rois)
                        else:
                            xform_out = RoIAlignFunction(
                                resolution, resolution, sc, sampling_ratio)(bl_in, rois)     
                    # print(blob_rois, lvl, rois.shape, xform_out.shape)
                    bl_out_list.append(xform_out)
                    # if blob_rois=='mask_rois':
                    #     # print(rpn_ret.keys())
                    # print(blob_rois, 'lvl_%d' % lvl, bl_in.shape, xform_out.shape)
                    # test_bl_list.append(xform_out)
            ...
            if isinstance(resolution, list):
                device_id = bl_out_list[0].get_device()
                restore_bl = rpn_ret[blob_rois + '_idx_restore_int32']
                restore_bl = Variable(
                    torch.from_numpy(restore_bl.astype('int64', copy=False))).cuda(device_id)
                for i in range(0, len(bl_out_list)):
                    assert device_id == bl_out_list[i].get_device(), "not in the same gpu device"
                    bl_out_list[i] = bl_out_list[i][restore_bl]
                    # print("unshuffled", bl_out_list[i].shape)
                xform_out = bl_out_list

            else:
                xform_shuffled = bl_out_list[0]
                for i in range(1, len(bl_out_list)):
                    xform_shuffled = torch.max(xform_shuffled, bl_out_list[i])
                ...
                # print(blob_rois, xform_shuffled.shape)

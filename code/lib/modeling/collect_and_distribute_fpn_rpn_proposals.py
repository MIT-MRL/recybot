...
def distribute(rois, label_blobs):
    ...
    for output_idx, lvl in enumerate(range(lvl_min, lvl_max + 1)):
        ...
        for i in range(lvl_max + 1 - lvl_min):
            if lvl == lvl_min:
                outputs[i+1] = rois[idx_lvl, :]
            else:
                outputs[i+1] = np.concatenate(
                                    (outputs[i+1],
                                    rois[idx_lvl, :]), axis=0)
    ...
    for lvl in range (lvl_min, lvl_max+1):
        rois_stacked = outputs[lvl - lvl_min + 1]
        # print(lvl, rois_stacked.shape)
